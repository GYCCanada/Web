...
>   because a single browser cannot start N hosted pages — **emails each registrant their OWN
>   Checkout `url`** via the `notifyPaymentLink` hook over the existing Mailer
>   (`registration-action.ts:317-339`, wired at `registration-route.ts:70-87`), and redirects the
>   submitter to an honest **"payment links sent — check your email"** success toast (NOT a Stripe
>   page). Each session reconciles independently on its own `checkout.session.completed`; persist
>   precedes notify, so a mail failure surfaces a form error WITHOUT losing the durable pending
>   sessions. No registrant is stranded.
...
| **(ii) payment cardinality** | `party._tag` as loop count | `group` ⇒ 1 `priceGroup` order/session (browser redirected to its hosted `url`); `perRegistrant` ⇒ N `priceRegistrant` orders/sessions — the action **emails each registrant their own Checkout `url`** (`notifyPaymentLink`, `registration-action.ts:317-339`) and redirects to a "payment links sent" toast (no Stripe redirect); each session settles independently. | C7/C7.5 |
...
- **Step 4 (create Checkout Sessions)** — the session's `customer_email` is read from the **frozen** `order.receiptEmail`, never re-read from form data. Idempotency keys unchanged (`registration:checkout:${requestFingerprint}:${mode}` / `…:perRegistrant:${index}`). The payer fields are now part of the fingerprinted payload (retry-stable; see 2b.6 note). For `group` the action **redirects** the browser (303) to the single session's hosted `url` — one party payment, complete (`registration-action.ts:333-334`). For `perRegistrant` (a single browser cannot start N hosted pages) the action instead **emails each registrant their own Checkout `url`** via `notifyPaymentLink` over the Mailer (`registration-action.ts:317-339`, `registration-route.ts:70-87`) and redirects to a "payment links sent" toast; each session reconciles independently on its `checkout.session.completed`. Persist-then-notify: a mail failure cannot lose the durable pending sessions.
...
- **Files:** EDIT `app/lib/content/pages/defaults.ts` (**author the `perRegistrant` option** onto `defaultRegistrationForm.party.billingMode.options` — C7a authored group-only; this is where the second mode becomes offered, AFTER the server branch exists, fixing the C7-standalone hazard). EDIT `registration-shell.ts` (widen to the **mode union** via `buildModeUnion`: add the `perRegistrant` arm + `requireRegistrantEmails`; keep the absent-mode default + present-off-list reject). EDIT `registration-action.ts` (branch on `decoded.party._tag`: `perRegistrant` ⇒ N orders/Checkout Sessions keyed `:${index}`, session `customer_email = registrants[i].email`; **email each registrant their own session `url`** via the `notifyPaymentLink` hook over the Mailer (`:317-339`) and redirect to a "payment links sent" toast — `group` redirects (303) to its single session `url` instead). EDIT `registration-form.tsx` (show the selector only when ≥2 modes authored; show the payer block only when the live mode is `group`).
